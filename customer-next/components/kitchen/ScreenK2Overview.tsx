        ...
        <div className="bg-stone-50 border-b-2 border-slate-900 p-3 shrink-0">
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {bulkAggregation.map((b, idx) => (
              <div
                key={idx}
                className="bg-white border-2 border-slate-900 rounded-lg p-2 shadow-[2px_2px_0px_#0f172a] flex flex-col justify-between"
              >
                <div className="flex items-center justify-between pb-1 border-b border-slate-200">
                  <span className="font-mono text-[11px] font-black text-slate-900 truncate">
                    [{b.name}]
                  </span>
                  <span className="font-mono text-[10px] font-black bg-slate-900 text-white px-1.5 py-0.2 rounded ml-1 shrink-0">
                    TOTAL: {b.total}
                  </span>
                </div>
                <div className="font-mono text-[9px] text-slate-600 truncate mt-1">
                  [{b.sources}]
                </div>
                <div className="font-mono text-[9px] font-extrabold text-orange-700 mt-0.5">
                  [{b.status}]
                </div>
              </div>
            ))}
