      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-1.5 mb-2.5 text-[9.5px] font-bold uppercase tracking-wider text-slate-400 font-mono">
            ...
            <span>[SPLIT PAYMENT OPTIONS]</span>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <motion.button
              whileTap={{ scale: 0.96 }}
              onClick={() => setSplitMode(payment.splitMode === 'ITEMS' ? 'NONE' : 'ITEMS')}
              className={`flex items-center justify-center gap-1.5 rounded-xl border py-2.5 px-2 text-[11px] font-extrabold tracking-wide transition ${
                payment.splitMode === 'ITEMS'
            ...
            <motion.button
              ...
              onClick={() =>
                setSplitMode(
                  payment.splitMode === 'PERSONS' ? 'NONE' : 'PERSONS',
                  splitPersons
                )
              }
              className={`flex items-center justify-center gap-1.5 rounded-xl border py-2.5 px-2 text-[11px] font-extrabold tracking-wide transition ${
                payment.splitMode === 'PERSONS'
                  ? 'border-slate-900 bg-slate-900 text-white shadow-xs'
                  : 'border-slate-200 bg-stone-50/70 text-slate-700 hover:bg-stone-100'
              }`}
            >
              <Users className="h-3.5 w-3.5" />
              <span>[SPLIT BY NUMBER OF PERSONS]</span>
            </motion.button>
          </div>
          ...
          {payment.splitMode === 'PERSONS' && (
            <motion.div
              ...
              className="mt-3 rounded-2xl border border-orange-200 bg-orange-50/50 p-3 text-xs"
            >
              <div className="flex items-center justify-between">
                <span className="font-bold text-slate-700">Dining Guests:</span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      const next = Math.max(2, splitPersons - 1);
                      setSplitPersons(next);
                      setSplitMode('PERSONS', next);
                    }}
                    className="flex h-6 w-6 items-center justify-center rounded-lg bg-white border border-slate-200 font-bold text-slate-800"
                  >
                    <Minus className="h-3 w-3" />
                  </button>
                  <span className="font-mono font-black text-slate-900 min-w-4 text-center">
                    {splitPersons}
                  </span>
                  <button
                    onClick={() => {
                      const next = splitPersons + 1;
                      setSplitPersons(next);
                      setSplitMode('PERSONS', next);
                    }}
                    className="flex h-6 w-6 items-center justify-center rounded-lg bg-white border border-slate-200 font-bold text-slate-800"
              ...
              <div className="mt-2 text-right font-extrabold text-orange-950 font-mono text-xs">
